...
=======
      <PaymentWebViewModal visible={!!state.paymentUrl} paymentUrl={state.paymentUrl} lang={state.lang}
        onClose={() => { state.setPaymentUrl(null); handlers.handleResetAll(); }}
        onPaymentSuccess={() => { state.setPaymentUrl(null); state.setIsOnlineSuccess(true); state.setVerificationSuccessModalVisible(true); }}
      />
      <DatePickerModal visible={state.datePickerVisible} onClose={() => state.setDatePickerVisible(false)} onConfirm={handlers.handleConfirmDate} pickerDay={state.pickerDay} setPickerDay={state.setPickerDay} pickerMonth={state.pickerMonth} setPickerMonth={state.setPickerMonth} pickerYear={state.pickerYear} setPickerYear={state.setPickerYear} adjustDay={handlers.adjustDay} adjustMonth={handlers.adjustMonth} adjustYear={handlers.adjustYear} />
      <SuccessModal visible={state.verificationSuccessModalVisible}
        onClose={() => { state.setVerificationSuccessModalVisible(false); handlers.handleResetAll(); }}
        title={state.isOnlineSuccess ? (state.lang === 'en' ? 'Payment Successful' : 'भुगतान सफल') : (state.lang === 'en' ? 'Request Submitted' : 'अनुरोध जमा किया गया')}
        message={state.isOnlineSuccess ? (state.lang === 'en' ? 'Your payment has been successfully processed. Dues are cleared.' : 'आपका भुगतान सफलतापूर्वक पूरा हो गया है।') : (state.lang === 'en' ? 'Your payment verification request has been submitted successfully.' : 'आपका भुगतान सत्यापन अनुरोध सफलतापूर्वक जमा कर दिया गया है।')}
>>>>>>> Stashed changes
      />
