...
describe('verifySupabaseOtp', () => {
  ...
  test('throws configuration error if Supabase credentials are missing', async () => {
    const config = require('../../../config');
    const originalUrl = config.supabaseUrl;
    config.supabaseUrl = '';

    await expect(verifySupabaseOtp('9876543210', '123456'))
      .rejects.toThrow('Supabase URL or Anon Key is not configured on the backend.');

    config.supabaseUrl = originalUrl; // restore
  });

  test('returns false if Supabase verification responds with non-200 status', async () => {
