...
describe('auth middleware', () => {
  ...
  describe('protected routes', () => {
    ...
    test('returns null verification if Supabase config is missing', async () => {
      const config = require('../../../config');
      const originalUrl = config.supabaseUrl;
      config.supabaseUrl = ''; // make it missing

      req.headers.authorization = 'Bearer sb-token';
      await authMiddleware(req, res, next);
      expect(res.status).toHaveBeenCalledWith(401);
    ...
    test('returns null verification if Supabase responds with non-200 code', async () => {
      req.headers.authorization = 'Bearer sb-token';
      axios.get.mockResolvedValue({
        status: 201, // not 200
        data: { id: 'sb-user-1' }
