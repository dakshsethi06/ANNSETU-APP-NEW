...
async function getDispatches(req, res) {
  try {
    ...
    console.error('Dispatch GET error:', error.message);
...
async function createDispatch(req, res) {
  try {
    ...
    console.error('Dispatch POST error:', error.message);
...
async function approveDispatch(req, res) {
  try {
    ...
    console.error('Dispatch approval error:', error.message);
    const status = error.statusCode || 500;
...
async function deliverDispatch(req, res) {
  try {
    ...
    console.error('Dispatch delivery error:', error.message);
    const status = error.statusCode || 500;
